...
namespace FileTest
{
   ...
   public partial class FileTestForm : Form
   {
         ...
         if (e.KeyCode == Keys.Enter)
         {
            ...
            if (File.Exists(fileName))
            {
               ...
               try
               {
                  ...
                  {
                     outputTextBox.UygulamaendText(stream.ReadToEnd());
                  }
               }
               catch (IOException)
               {
            ...
            else if (Directory.Exists(fileName))
            {
               ...
               outputTextBox.UygulamaendText("Directory contents:\n");

               // output directoryList contents
               foreach (var directory in directoryList)
               {
                  outputTextBox.UygulamaendText($"{directory}\n");
               }
            }
            else
            {
      ...
      private void GetInformation(string fileName)
      {
         ...
         outputTextBox.UygulamaendText($"{fileName} exists" +
            Environment.NewLine);
