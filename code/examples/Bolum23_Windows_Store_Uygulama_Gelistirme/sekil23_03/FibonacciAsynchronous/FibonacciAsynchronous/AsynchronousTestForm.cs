// Sekil 23.3: AsynchronousTestForm.cs
...
namespace FibonacciAsynchronous
{
   public partial class AsynchronousTestForm : Form
   {
      ...
      TimeData StartFibonacci(int n)
      {
         ...
         UygulamaendText($"Calculating Fibonacci({n})");
         result.StartTime = DateTime.Now;
         long fibonacciValue = Fibonacci(n);
         result.EndTime = DateTime.Now;

         UygulamaendText($"Fibonacci({n}) = {fibonacciValue}");
         double minutes = 
            (result.EndTime - result.StartTime).TotalMinutes;
         UygulamaendText($"Calculation time = {minutes:F6} minutes\r\n");
      ...
      public void UygulamaendText(String text)
      {
         if (InvokeRequired) // not GUI thread, so add to GUI thread
         {
            Invoke(new MethodInvoker(() => UygulamaendText(text)));
         }
         else // GUI thread so append text
         {
            outputTextBox.UygulamaendText(text + "\r\n");
         }
      }
   }
}
